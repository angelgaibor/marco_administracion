...
enlistamiento_01 <- enlistamiento |> 
  filter(!substr(upm, 1, 10) %in% unique(substr(marco_old1$id_upm[marco_old1$cod_cart == 2], 1, 10))) |> 
...
table(redistribucion_02$id_upm4)
...
pluscode_perdido <- readRDS("insumos/00_viviendas/enighur/pluscode_perdido.rds") |> 
  mutate(id_viv_car = paste0(substr(id_viv, 1, 18), substr(id_viv, 20, 22)))

enlistamiento_06 <- enlistamiento_05 |> 
  left_join(pluscode_perdido |> 
              select(id_viv_car, pluscodes_lost = pluscodes),
            by = "id_viv_car") |> 
  mutate(pluscodes = case_when(is.na(pluscodes) ~ pluscodes_lost,
                                T ~ pluscodes),
         nap_lost = runif(23269),
         nap = case_when(is.na(nap) ~ nap_lost,
                         T ~ nap),
         link = case_when(is.na(link) ~ pluscodes,
                          T ~ link)) |> 
  arrange(id_upm, nap) |> 
  group_by(id_upm) |> 
  mutate(orden_lost = n()) |> 
  ungroup() |> 
  mutate(orden = case_when(is.na(orden) ~ orden_lost,
                           T ~ orden)) |> 
  select(-pluscodes_lost, -nap_lost, -orden_lost)
  
colSums(is.na(enlistamiento_06))

table(enlistamiento_06$orden, enlistamiento_06$seleccion)

names(enlistamiento_06)[!names(enlistamiento_05) %in% names(marco_old1)]
names(marco_old1)[!names(marco_old1) %in% names(enlistamiento_06)]

marco_viviendas_01 <- marco_old1 |> 
  rename(encuesta = encueta) |> 
  mutate(id_viv_car = paste0(pro, can, par, zon, sec, manloc, n_edif_umce, n_viv)) |> 
  select(names(enlistamiento_06)) |> 
  rbind(enlistamiento_06) |> 
