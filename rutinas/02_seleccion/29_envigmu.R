...
print("Oye programador promedio reloadad (PPR), no te olvides de actualizar el aux_seleccionable") #comprobar que todos los aux se encuesntren en base
...
if(sum(grepl(paste0(encuesta,"_", periodo), names(marco))) == 0){
  ...
  marco_01 <- marco |> 
    ...
    mutate(aux_estrato = ifelse(Mi < Mi_min, "9999", estrato),
           aux_seleccionable = case_when(enciet_202511_sel == 1 ~ 17,
                                         enciet_202510_sel == 1 ~ 16,
