...
if(sum(grepl(paste0(encuesta,"_",periodo), names(marco))) == 0){
  ...
  marco_01 <- marco |> 
    ...
    mutate(aux_estrato = ifelse(Mi < Mi_min, "9999", estrato),
           aux_seleccionable = case_when(enciet_202508_sel == 1 ~ 14,
...
table(marco_01$enciet_202411_sel, marco_01$enciet_202509_sel)
table(marco_01$enighur_sel, marco_01$enciet_202509_sel)
table(marco_01$endi3_sel, marco_01$enciet_202509_sel)
table(marco_01$steps_202506_sel, marco_01$enciet_202509_sel)

solapamiento_enighur <- marco_01 %>% 
  filter(enighur_sel == 1 & enciet_202509_sel == 1) %>% 
  mutate(encuesta = "enighur") |> 
  select(id_upm, pro, area, domgeo, domest, estrato, encuesta)

solapamiento_endi <- marco_01 %>% 
  filter(endi3_sel == 1 & enciet_202509_sel == 1) %>% 
...
saveRDS(solapamiento, "../ENCIET/intermedios/03_muestra_upm/a25m09/solapamiento_enighur.rds")
